...
class HeartCodec(PreTrainedModel):
    ...
    def detokenize(
        ...
        # Preserve the model's native dtype throughout to avoid FP32 / BF16 conflicts.
        ...
        first_latent = torch.randn(
            codes.shape[0], int(duration * 25), 256,
        ...
        # Pad codes to the minimum required length
        ...
        # Pad to the nearest multiple of hop_samples
        ...
        for sinx in range(0, codes.shape[-1] - hop_samples + 1, hop_samples):
            ...
            if sinx == 0 or ovlp_frames == 0:
                latents = self.flow_matching.inference_codes(
                    codes_input, first_latent, latent_length, first_latent_length,
                    guidance_scale=guidance_scale, num_steps=num_steps,
                    disable_progress=disable_progress, scenario="other_seg",
                )
                ...
                true_latent = torch.cat([true_latent, noise], dim=1)
                latents = self.flow_matching.inference_codes(
        ...
        # Keep everything in the model's native dtype during decoding
        latent_list = [lat.to(target_dtype) for lat in latent_list]
        ...
        for latent in latent_list:
            ...
            # Reshape to two-band stereo-like layout expected by the scalar model
            latent = (
                latent.reshape(bsz, t, 2, f // 2)
                .permute(0, 2, 1, 3)
                .reshape(bsz * 2, t, f // 2)
            )
            cur_output = (
            ...
            cur_output = cur_output[:, :min_samples].detach().cpu()
            if cur_output.dim() == 3:
            ...
            if output is None:
                ...
                if ovlp_samples == 0:
                    output = torch.cat([output, cur_output], dim=-1)
                else:
                    ov_win = torch.from_numpy(np.linspace(0, 1, ovlp_samples)[None, :])
                    ov_win = torch.cat([ov_win, 1 - ov_win], dim=-1)
                    output[:, -ovlp_samples:] = (
                        output[:, -ovlp_samples:] * ov_win[:, -ovlp_samples:]
                        + cur_output[:, :ovlp_samples] * ov_win[:, :ovlp_samples]
                    )
                    output = torch.cat([output, cur_output[:, ovlp_samples:]], dim=-1)

        output = output[:, :target_len]

        # Always return float32 for WAV file compatibility
