...
class RotaryEmbedding(nn.Module):
    """RoPE with an internal sine/cosine cache."""
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is None:
            ...
            _, tk, _ = encoder_hidden_states.shape
        ...
        sin, cos = self.rope.get_sin_cos(seq_len_for_rope, device=x.device, dtype=x.dtype)

        def apply_rope_vec(tensor: torch.Tensor) -> torch.Tensor:
            head = tensor[..., :rope_dim]
            tail = tensor[..., rope_dim:]
            bh, h, tt, _ = head.shape
            head = head.view(bh, h, tt, rope_dim // 2, 2)
            sin_ = sin.view(1, 1, tt, rope_dim // 2, 1)
            cos_ = cos.view(1, 1, tt, rope_dim // 2, 1)
            x1, x2 = head[..., 0:1], head[..., 1:2]
            rot = torch.cat(
                [x1 * cos_ - x2 * sin_, x1 * sin_ + x2 * cos_], dim=-1
            ).view(bh, h, tt, rope_dim)
...
class LlamaTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn = LlamaAttention(
            ...
            bias=attention_bias, dropout=dropout,
            rope_dim=head_dim, cross_attention_dim=None,
        )
        ...
        if cross_attention_dim is not None:
            ...
            self.cross_attn = LlamaAttention(
                dim, n_heads, head_dim,
                bias=attention_bias, dropout=dropout,
                rope_dim=head_dim, cross_attention_dim=cross_attention_dim,
            )
    ...
    def forward(
        ...
        if self.use_ada_layer_norm_single:
            ...
            h = self.attn(self.attn_norm(x) * (1 + scale_msa) + shift_msa, attention_mask=attention_mask)
            x = x + gate_msa * h
            h = self.mlp(self.mlp_norm(x) * (1 + scale_mlp) + shift_mlp)
            x = x + gate_mlp * h
        else:
...
class ProjectLayer(nn.Module):
    def __init__(self, hidden_size: int, filter_size: int, kernel_size: int = 1, dropout: float = 0.0):
        super().__init__()
...
class LlamaTransformer(nn.Module):
    ...
    def forward(
        ...
        timestep_mod = embedded_timestep = None
        ...
        s = self.norm_out(s) * (1 + scale) + shift

        x = self.connection_proj(torch.cat([hidden_states, s], dim=-1))

        timestep_mod_2 = embedded_timestep_2 = None
        if timestep is not None:
        ...
        x = self.norm_out_2(x) * (1 + scale_2) + shift_2
...
class PixArtAlphaCombinedFlowEmbeddings(nn.Module):
    ...
    def timestep_embedding(self, timesteps: torch.Tensor, max_period: int = 10000, scale: int = 1000) -> torch.Tensor:
