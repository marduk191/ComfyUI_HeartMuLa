...
# Scripting this activition brings a ~1.4x speed-up on long sequences.
@torch.jit.script
    ...
    def __init__(
        ...
        if causal:
            ...
            assert kernel_size == 2 * stride, (
                "kernel_size must equal 2*stride in causal ConvTranspose1d."
            )
        super().__init__(
            in_channels,
            out_channels,
...
class round_func9(InplaceFunction):
    """Straight-through quantiser with 9 discrete levels."""
...
class ScalarModel(nn.Module):
    def __init__(
        ...
        if num_samples > 1:
            encoder_layers.append(
                PreProcessor(
                    init_channel, init_channel, num_samples,
                    kernel_size=default_kernel_size, causal=causal,
                )
            )
        for i, down_factor in enumerate(downsample_factors):
            encoder_layers.append(
                ResEncoderBlock(
                    init_channel * int(np.power(2, i)),
                    init_channel * int(np.power(2, i + 1)),
                    down_factor,
        ...
        encoder_layers.append(
            weight_norm(
                Conv1d(
                    init_channel * int(np.power(2, len(downsample_factors))),
                    latent_hidden_dim,
        ...
        decoder_layers.append(
            weight_norm(
                Conv1d(
                    latent_hidden_dim,
                    init_channel * int(np.power(2, len(upsample_factors))),
        ...
        for i, upsample_factor in enumerate(upsample_factors):
            decoder_layers.append(
                ResDecoderBlock(
                    init_channel * int(np.power(2, len(upsample_factors) - i)),
                    init_channel * int(np.power(2, len(upsample_factors) - i - 1)),
                    upsample_factor,
    ...
    def _encode(self, x: torch.Tensor) -> torch.Tensor:
        """Shared encoder pass; returns tanh-activated latent."""
        for i, layer in enumerate(self.encoder):
            if i != len(self.encoder) - 1:
                x = layer(x)
            else:
                # torch.tanh is the preferred form; the functional alias was deprecated in PyTorch 1.x
    ...
    def inference(self, x: torch.Tensor):
        ...
        for layer in self.decoder:
            out = layer(out)
        return emb, emb_quant, out

    def encode(self, x: torch.Tensor) -> torch.Tensor:
        return self._encode(x)

    def decode(self, x: torch.Tensor) -> torch.Tensor:
        # Quantise to ensure predictions follow the same distribution as training
        x = self.vq.apply(x)
        for layer in self.decoder:
            x = layer(x)
        return x
